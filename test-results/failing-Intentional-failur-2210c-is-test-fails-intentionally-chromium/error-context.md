...
- Name: failing.spec.js >> Intentional failures for CI practice >> this test fails intentionally
- Location: tests\failing.spec.js:10:3
...
```ts
  ...
  2  | 
  3  | test.describe('Intentional failures for CI practice', () => {
  4  | 
  5  |   test('this test passes', async ({ page }) => {
  6  |     await page.goto('/');
  7  |     await expect(page).toHaveTitle(/Playwright/);
  8  |   });
  9  | 
  10 |   test('this test fails intentionally', async ({ page }) => {
  11 |     await page.goto('/');
> 12 |     await expect(page).toHaveTitle(/This Title Does Not Exist/);
     |                        ^ Error: expect(page).toHaveTitle(expected) failed
  13 |   });
  14 | 
  15 | });
